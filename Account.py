class Account:
    ...
    def display(self):
        ...
        length = max(len(f"Account Balance: {self.balance}"), len(f"Id: {self.id}"), len("Transaction History"), *[len(f"{transaction[0]}: {transaction[1]}") for transaction in self.list_of_transectios.items()])
        ...
        print(f"Id: {self.id}")
        if len(self.list_of_transectios) == 0:
            print("No transactions yet.")
        print()
        print("-" * length)
        print("Transaction History")
        print("-" * length)
        for transaction in self.list_of_transectios.items():
